...
const createWindow = async () => {
  ...
  const prevBounds = store.get("mainWindowBounds", {
    width: 1024,
    height: 768,
    x: -1,
    y: -1,
  ...
  if (prevBounds.x >= 0 && prevBounds.y >= 0) {
    ...
    mainWindow.setSize(prevBounds.width, prevBounds.height);
    mainWindow.center();
  }
...
app.on("ready", async () => {
  ...
  const tlshotApi = new TlshotApi();
  TlshotApi.connect(tlshotApi);
