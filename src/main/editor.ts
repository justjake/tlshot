...
import { STORE, WindowHistoryLog } from "./services";
...
export async function createEditorWindow() {
  ...
  const prevBounds = STORE.get("editorWindowBounds", {
    width: 1024,
    height: 768,
    x: -1,
    y: -1,
  ...
  editorWindow.on("close", () => {
    ...
    STORE.set("editorWindowBounds", editorWindow.getBounds());
  });
  ...
  // Open the DevTools.
  if (STORE.get("editorWindowDevtools", false)) {
    editorWindow.webContents.openDevTools();
  }

  WindowHistoryLog.track(editorWindow);
